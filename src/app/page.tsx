...
export default function Home() {
  ...
  const [categoriesOpenTaskId, setCategoriesOpenTaskId] = useState<string | null>(null);
  ...
  const fetchTasksAndCategories = async (userId: string | null) => {
    try {
  ...
  const addTask = async () => {
    ...
    if (response.ok) fetchTasksAndCategories(userId);
    setInput("");
  };

  const toggleCompleteAndAssignCategory = async (id: String, completed: Number, category: String, categoryTask: String) => {
    await fetch(`/api/tasks/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ completed: completed === 0 && (category == "" || category == categoryTask) ? 1 : 0, type: "Task", category: category}),
    });
    fetchTasksAndCategories(userId);
  };
  ...
  <ol>
    {tasks.map((task) => (
      ...
      >
        ...
        <button
        onClick={() =>
          setCategoriesOpenTaskId(categoriesOpenTaskId === task._id ? null : task._id)
        }
        className={`p-1 rounded-full bg-gray-200 text-gray-700 relative group`}
            ...
            <div className="absolute right-full mr-4 top-0 w-64 bg-white shadow-md p-4 rounded-lg">
              ...
              <ul>
                {categories.map((category) => (
                  <li key={String(category._id)} className="flex justify-between items-center py-2 border-b border-gray-300 relative">
                    <div className="flex items-center space-x-2">
                    <span className="text-sm">
                    {category.name}
                    </span>
                    <span className="p-1 rounded group" style={{
                    ...
                    <button
                    onClick={() => toggleCompleteAndAssignCategory(task._id, task.completed, category.name, "")}
          ...
          {task.category !== "" && (
          <button className="p-1 rounded relative group" style={{
                      backgroundColor: categories.find(c => c.name === task.category)?.color,
                      width: "20px",
                      height: "20px",
          }}>
          <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-1 bg-gray-700 text-white text-xs rounded px-2 py-1 opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity duration-200 z-20">
        ...
        <div className="relative flex items-center space-x-2">
          <button
            onClick={() => toggleCompleteAndAssignCategory(task._id, task.completed, String(categories.find(c => c.name === task.category)?.name) ? String(categories.find(c => c.name === task.category)?.name) : "", task.category)}
            className={`relative p-1 rounded ${
              task.completed
